...
Ext.define('PdfViewer.view.combo.scaleCombo', {extend:'Ext.form.field.ComboBox', xtype:'PdfViewerScaleCombo', requires:['Ext.data.ArrayStore'], editable:false, keyNavEnabled:true, selectOnFocus:false, submitValue:false, isFormField:false, autoSelect:true, store:new Ext.data.SimpleStore({id:0, fields:['scale', 'text'], data:[[0.5, '50%'], [0.75, '75%'], [1, '100%'], [1.25, '125%'], [1.5, '150%'], [2, '200%']]}), valueField:'scale', displayField:'text', mode:'local', listeners:{change:function(cb, 
newVal) {
  var record = cb.store.findRecord('scale', newVal);
  if (record === null) {
    cb.store.add({scale:newVal, text:Math.round(newVal * 100).toString() + '%'});
  }
}}});
Ext.define('PdfViewer.view.field.pageNumber', {extend:'Ext.form.field.Number', xtype:'pdfviewer_pagenumber', allowDecimals:false, minValue:1, hideTrigger:true, enableKeyEvents:true, keyNavEnabled:false, selectOnFocus:true, submitValue:false, isFormField:false});
Ext.define('PdfViewer.view.panel.PDFController', {extend:'Ext.app.ViewController', alias:'controller.PdfViewerController', requires:[], container:null, pageScale:null, zoomFactor:null, minScale:null, maxScale:null, control:{'#':{boxready:'onBoxReady', onSetSrc:'onSetSrc', onUnset:'onUnset', onGetDocument:'onGetDocument', afterrender:'onAfterrender'}}, init:function(view) {
  var me = this;
  ...
  var me = this, view = me.getView(), isEmpty, pageCount, currPage, afterText;
  if (view.isRendering) {
    return;
  }
  ...
  for (var i = idx; i <= numPages; i++) {
    view.pdfDoc.getPage(i).then(function(page) {
      ...
      page.render(renderContext).then(function() {
        ...
        PDFJS.renderTextLayer({textContent:textContent, container:textLayerDiv, viewport:viewport, pageIndex:page.pageIndex, textDivs:[]});
      });
      ...
      if (view.rendered) {
        ...
        view.scrollTo(0, 0);
        view.fireEvent('change', view, {current:view.currentPage, total:view.pdfDoc.numPages});
      }
    });
  }
}, moveFirst:function() {
  ...
  var view = this.getView(), prev = view.currentPage - 1;
  if (prev > 0) {
  ...
  var view = this.getView(), total = view.pdfDoc.numPages, next = view.currentPage + 1;
  if (next <= total) {
  ...
  var me = this.getView(), last = me.pdfDoc.numPages;
  ...
  var me = this.getView(), k = e.getKey(), increment = e.shiftKey ? 10 : 1, pageNum, total = me.pdfDoc.numPages;
  if (k === e.RETURN) {
    e.stopEvent();
  ...
  var view = this.getView(), v = this.lookup('inputItem').getValue(), pageNum = parseInt(v, 10);
  if (!v || isNaN(pageNum)) {
    this.lookup('inputItem').setValue(view.currentPage);
    return false;
  }
  return pageNum;
}, privates:{setToolbarProperties:function(page, reset) {
  var me = this, view = me.getView(), currPage = page, pageCount = view.pdfDoc.numPages, isEmpty = pageCount === 0, afterText = Ext.String.format(view.afterPageText, isNaN(pageCount) ? 1 : pageCount);
  ...
  view.loader = new Ext.LoadMask({msg:view.loadingMessage, target:view});
  view.loader.show();
...
Ext.define('PdfViewer.view.panel.PDF', {extend:'Ext.panel.Panel', controller:'PdfViewerController', xtype:'PdfViewerPdfPanel', requires:['Ext.button.Button', 'Ext.toolbar.Fill', 'Ext.toolbar.Separator', 'Ext.toolbar.TextItem', 'PdfViewer.view.combo.scaleCombo', 'PdfViewer.view.field.pageNumber', 'PdfViewer.view.panel.PDFController'], extraBaseCls:Ext.baseCSSPrefix + 'pdf', extraBodyCls:Ext.baseCSSPrefix + 'pdf-body', autoScroll:true, disableTextLayer:false, showPerPage:false, pageScale:1, pageBorders:true, 
src:'', disableWorker:false, loadingMessage:'Loading PDF, please wait...', beforePageText:'Page', afterPageText:'of {0}', firstText:'First Page', prevText:'Previous Page', nextText:'Next Page', lastText:'Last Page', inputItemWidth:60, scaleWidth:100, showLoadMaskOnInit:null, zoomFactor:0.1, minScale:0.5, maxScale:4, initComponent:function() {
  var me = this, dockedItems;
  dockedItems = [{dock:'bottom', reference:'PagingToolbar', xtype:'toolbar', items:[{reference:'first', tooltip:me.firstText, overflowText:me.firstText, iconCls:'ext ext-double-chevron-left', disabled:true, handler:'moveFirst'}, {reference:'prev', tooltip:me.prevText, overflowText:me.prevText, iconCls:'ext ext-chevron-left', disabled:true, handler:'movePrevious'}, '-', me.beforePageText, {xtype:'pdfviewer_pagenumber', reference:'inputItem', name:'inputItem', cls:Ext.baseCSSPrefix + 'tbar-page-number', 
  width:me.inputItemWidth, margins:'-1 2 3 2', disabled:true, listeners:{keydown:'onPagingKeyDown', blur:'onPagingBlur'}}, {xtype:'tbtext', reference:'afterTextItem', text:Ext.String.format(me.afterPageText, 1), margins:'0 5 0 0'}, '-', {reference:'next', tooltip:me.nextText, overflowText:me.nextText, iconCls:'ext ext-chevron-right', disabled:true, handler:'moveNext'}, {reference:'last', tooltip:me.lastText, overflowText:me.lastText, iconCls:'ext ext-double-chevron-right', disabled:true, handler:'moveLast'}, 
  '-\x3e', {xtype:'button', iconCls:'fa fa-search-plus', tooltip:'Zoom in', handler:'onBtnZoomInClicked'}, {reference:'scaleCombo', xtype:'PdfViewerScaleCombo', disabled:true, width:me.scaleWidth, listeners:{change:'onScaleChange', blur:'onScaleBlur'}}, {xtype:'button', tooltip:'Zoom out', iconCls:'fa fa-search-minus', handler:'onBtnZoomOutClicked'}]}];
  if (typeof me.dockedItems !== 'undefined') {
    dockedItems = dockedItems.concat(me.dockedItems);
  }
  Ext.apply(me, {html:'\x3cdiv class\x3d"canvasWrapper"\x3e\x3c/div\x3e', dockedItems:dockedItems});
